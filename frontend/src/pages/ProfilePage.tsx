      ...
      <div className="flex items-center gap-3">
        ...
        {!editing && (
          <button
            ...
            className="ml-auto text-xs border border-gray-300 px-3 py-1.5 rounded text-gray-600 hover:bg-gray-50 transition cursor-pointer"
          >
            수정
          </button>
        )}
      ...
      {editing && (
        <form onSubmit={save} className="space-y-3 pt-1">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => { setEditing(false); setForm({ name: user.name, jobTitle: user.jobTitle ?? '' }); }}
...
function IntegrationsSection() {
  ...
  const connectedTypes = new Set(integrations.map(i => i.serviceType));

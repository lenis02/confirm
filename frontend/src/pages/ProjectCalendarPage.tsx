...
type CalendarEvent = {
  id: string;
  type: 'meeting' | 'action' | 'milestone' | 'project-start' | 'project-end';
  label: string;
};
...
function buildEventMap(
  ...
  wbs?.items
    ...
    .filter(i => deptFilter === '전체' || i.assignedRole === deptFilter)
    .forEach(i => {
...
export default function ProjectCalendarPage() {
  ...
  const deptOptions = ['전체', ...Array.from(new Set(
    (wbs?.items ?? []).filter(i => i.isDecisionPoint && i.endDate && i.assignedRole).map(i => i.assignedRole)
  ))];
  ...
  if (project.startDate && project.endDate) {
    ...
    progress = Math.min(100, Math.max(0, Math.round(((cur - start) / (end - start)) * 100)));
  }
    ...
    <div className="p-6 max-w-5xl mx-auto">
        ...
        <div className="p-4">
          <div className="flex items-start justify-between gap-4 mb-3">
            ...
            <Link
              ...
              className="shrink-0 text-xs border border-gray-300 text-gray-500 px-2.5 py-1.5 rounded hover:bg-gray-50 transition"
            >
              상세 보기
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-xs text-gray-500">
            <div>
              <p className="text-gray-400 mb-0.5">시작일</p>
              <p className="font-medium text-gray-700">{project.startDate ?? '미정'}</p>
            </div>
            <div>
              <p className="text-gray-400 mb-0.5">종료일</p>
              <p className="font-medium text-gray-700">{project.endDate ?? '미정'}</p>
            </div>
          </div>
      ...
      {deptOptions.length > 1 && (
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <span className="text-xs text-gray-400 shrink-0">WBS 마일스톤 필터</span>
